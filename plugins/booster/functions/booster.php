...
function post_boost_create($val, $result, $admin = false) {
    ...
    if (validation_passes()) {
        ...
        $ad_type = "";
        if($booster_type == 'feed') $ad_type = 'Post';
        if($booster_type == 'listing') $ad_type = 'Listing';
        if($booster_type == 'Product') $ad_type = 'Product';
        if($booster_type == 'property') $ad_type = 'property';
            ...
            $result['message'] = lang('booster::you_have_successfully',array('type'=>$booster_type));
        ...
        if (is_admin()) {
            db()->query("UPDATE `post_boost` SET status=1,paid=1 WHERE ads_id='{$adsId}'");
        }
...
function count_user_boost_total($type) {
    ...
    switch($type) {
        ...
        case 'views':
            ...
            break;
        case 'clicks':
            $sql = "SELECT SUM(click_stats) as size FROM `post_boost` ";
            break;
    }
    $userid = get_userid();
...
function get_boosted_listings($type='Listing'){
    $sql = "SELECT `post_id` FROM `post_boost` WHERE `type` = '{$type}'";
    $result = db()->query($sql);
    $posts = array();
    while($r = $result->fetch_assoc()){
        $posts[] = $r['post_id'];
