...
function post_boost_pager($app){
    ...
    switch($type){
        ...
        case 'Listing':
            ...
            break;
        case 'Product':
            $product = getSingleProduct($p_id);
            if(!empty($product)){
                $_SESSION['booster_type'] = 'Product';
                echo view('booster::product',array('product'=>$product));
            }
            break;
        case 'property':
            $property = property_get_property($p_id);
            if(!empty($property)){
                $_SESSION['booster_type'] = 'property';
                echo view('booster::property',array('property'=>$property));
            }
            break;
    }
        ...
        case 'paypal':
            ...
            switch($type) {
                case 'request':
					...
					$paypal->add_field('csrf_token', $csrf_token);
                    $paypal->add_field('return', url_to_pager('pb_activate', array('id' => $bp['pb_id'])).'?action=paypal&type=success');
                    $paypal->add_field('cancel_return', url_to_pager('pb_activate', array('id' => $bp['pb_id'])).'?action=paypal&type=cancel');
                    ...
                    $paypal->add_field('item_name', $bp['name']);
                    $paypal->add_field('item_number', $bp['pb_id']);
...
function boost_paypal_process_pager($app) {
    ...
    if($paypal->validate_ipn()) {
        ...
        fire_hook("payment.aff",null,array('booster',$id));
...
function booster_payment_success_pager($app) {
    //CSRFProtection::validate();
    //$id = input('id');
    //$bp = find_pb($id);
    //if (!$bp or $bp['user_id'] != get_userid()) return redirect_to_pager('manage-boost');
    //activate_page_boost($bp);
    $app->setTitle(lang("success"));
    return $app->render(view('booster::payment/success'));
}

function booster_payment_cancel_pager($app) {
    $app->setTitle(lang('booster::canceled'));
    return $app->render(view('booster::payment/cancel'));
}