    ...
    * Payment received successfully - https://coins.gourl.io/b/20/c/Bitcoin/p/20AAvZCcgBitcoin77BTCPUB0xyyeKkxMUmeTJRWj7IZrbJ0oL/a/0/au/0.1/pe/NOEXPIRY/l/en/o/invoice1/u/demo/us/MANUAL/j/1/d/ODIuMTEuOTQuMTIx/h/ac7733d264421c8410a218548b2d2a2a
    ...
    public function display_cryptobox_bootstrap($coins = array(), $def_coin = "", $def_language = "en", $custom_text = "", $coinImageSize = 70, $qrcodeSize = 200, $show_languages = true, $logoimg_path = "default", $resultimg_path = "default", $resultimgSize = 250, $redirect = "", $method = "curl", $debug = false) {
        ...
        if ($method == "ajax") {
            ...
            $tmp .= '<script>document.addEventListener("DOMContentLoaded", function(){cryptobox_update_page("'.base64_encode($data).'", "'.base64_encode($imgdir_path).'", "'.base64_encode($logoimg_path).'", "'.base64_encode($ext).'") })</script>';
        ...
        $tmp .= "<button type='submit' title='Click to Reload Page' class='btn btn-outline-secondary btn-lg'><i class='fa fa-spinner fa-spin'></i> &#160; ".$this->coin_name()." ".$this->localisation["loading"]."</button>";
        $tmp .= "</div>";
        ...
        $tmp .= "<span class='".$ext."loading_icon mr-3 float-left' ".$hide."> <i class='fa fa-laptop'></i></span>";
        $tmp .= "<span class='".$ext."loading_icon mr-3 float-left' ".$hide."> <i class='fa fa-sync-alt fa-spin'></i></span>";
        $tmp .= "</h4>";
        $tmp .= "</div>";
        ...
        $tmp .= "<br><br>";
        $tmp .= "<button type='submit' class='".$ext."button_error btn btn-outline-primary btn-block btn-lg'><i class='fa fa-sync'></i> &#160; Reload Page</button>";
        $tmp .= "<br>";
        $tmp .= "</div>";
        $tmp .= "</div>";
        $tmp .= "<br><br><br><br><br>";
        ...
        $tmp .= "<button type='submit' class='".$ext."refresh btn btn-sm btn-outline-secondary float-right'><i class='fa fa-sync-alt'></i></button>";
        $tmp .= "<span class='".$ext."loading_icon mr-3 float-left' ".$hide."> <i class='fa fa-laptop'></i></span>";
        $tmp .= "<span class='".$ext."loading_icon mr-3 float-left' ".$hide."> <i class='fa fa-sync-alt fa-spin'></i></span>";
        $tmp .= "</h4>";
        ...
        $tmp .= "<a class='".$ext."copy_address' href='#a'><i class='fa fa-copy'></i></a> &#160;&#160;";
        $tmp .= "<a class='".$ext."wallet_open' href='#a'><i class='fa fa-external-link-alt'></i></a>";
        $tmp .= "</h4>";
        $tmp .= "<br>";
        ...
        if ($method != "ajax" && !$this->is_paid()) {
            ...
            $tmp .= "<br><button type='submit' class='".$ext."button_confirm btn btn-lg btn-block btn-primary my-2' style='white-space:normal'><i class='fa fa-angle-double-right'></i> &#160; ".str_replace(array("%coinName%", "%coinNames%", "%coinLabel%"), array($this->coinName, (in_array($this->coinLabel, array('BCH', 'BSV', 'DASH')) ? $this->coinName : $this->coinName.'s'), $this->coinLabel), $this->localisation["button"])." &#160; <i class='fa fa-angle-double-right'></i></button>";
            $tmp .= "</form>";
            $tmp .= "</div>";
        }
...
function cryptobox_sellanguage($default = "en") {
    ...
    if (isset($_GET[$id]) && in_array($_GET[$id], array_keys($localisation)) && !defined("CRYPTOBOX_LANGUAGE_HTMLID_IGNORE")) {
        $lan = $_GET[$id];
...
function cryptobox_selcoin($coins = array(), $default = "") {
    ...
    if (isset($_GET[$id]) && in_array($_GET[$id], $available_payments) && in_array($_GET[$id], $coins)) {
        $coinName = $_GET[$id];
