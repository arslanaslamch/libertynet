...
function cryptobox_new_payment($payment_id = 0, $payment_details = array(), $box_status = '') {
    list($type, $type_id, $user_id) = explode('-', $payment_details['order']);
    fire_hook('payment.aff', null, array($type, $type_id, $user_id));

    /*$log = '['.date('Y-d-m H:i:s').'] Payment - '.$payment_id.' - '.$box_status.':'."\n".'Payment ID: '.$payment_id."\n\n".'Status: '.$box_status."\n\n".'Details: '.print_r($payment_details, true);
    $temp_dir = config('temp-dir', path('storage/tmp')).'/crptopay';
    $file_name = 'log';
    if (!is_dir($temp_dir)) {
        mkdir($temp_dir, 0777, true);
    }
    $temp_path = $temp_dir.'/'.$file_name;
    file_put_contents($temp_path, $log, FILE_APPEND);*/
    return true;
}