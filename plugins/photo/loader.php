...
register_hook('uploader.confirm.file', function($validate, $type, $source_uri) {
    if(config('prevent-nude-photo-upload', false) && $type == 'image') {
        $nudity_details = photo_nudity_details($source_uri);
        if($nudity_details && $nudity_details['status'] == 'success' && $nudity_details['nudity']['raw'] * 100 > config('photo-nudity-tolerance', 50)) {
            $validate['status'] = false;
            $validate['message'] = lang('photo::upload-nudity-error');
        }
    }
    return $validate;
});