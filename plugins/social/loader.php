...
register_hook('account.settings.menu', function() {
	add_menu("account-menu", array("id" => "invite", "link" => url_to_pager("account").'?action=invite', "title" => lang("find-friends"), "icon" => array("class" => "fa fa-search-plus", "color" => "#4eabf4")));
});

register_hook('account.settings', function($action) {
	if($action == 'invite') {
		app()->setTitle(lang('social::invite-friends'));
        get_menu("account-menu", "invite")->setActive();
		$emails = input('emails');
		$message = null;
		if($emails) {
