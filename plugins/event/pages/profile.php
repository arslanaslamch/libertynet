...
function event_profile_pager($app) {
	...
	set_meta_tags(array('name' => get_setting("site_title", "Crea8social"), 'title' => get_event_details('event_title'), 'description' => get_event_details('event_desc'), 'image' => get_event_details('event_cover') ? url_img(get_event_details('event_cover'), 920) : '', 'keywords' => ''));
...
function rsvp_pager($app) {
    ...
    $users = getEventAudience($eventId, ($rsvp == 0)?'1':$rsvp, false, 30);
	$audience = view('event::profile/load-audience', array('users' => $users, 'event' => find_event($eventId)));
...
function event_audience_pager($app){
    $app->setTitle(lang('event::audience'));
    $id = segment(1);
    $event = find_event($id);
    if ($id and $event){
        $rsvp = input('rsvp',1);
        $users = ($rsvp != 3)?getEventAudience($id, $rsvp,true):get_event_invited($id);
        return $app->render(view('event::profile/audience',array('users' => $users, 'event' => $event)));
    }
    redirect_back();
}