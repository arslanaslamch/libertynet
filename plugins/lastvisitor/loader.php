...
register_hook('notification.pusher.notifications', function($pusher, $notification, $template) {
    $result = $template;
    $type = $notification['type'];
    if($type == 'profile.view') {
        $user = get_user();
        $result['status'] = true;
        $result['options']['title'] = lang('lastvisitor::profile-view');
        $result['options']['body'] = isset($notification['title']) && $notification['title'] == 'again' ? get_user_name($notification['from_userid']).' '.lang('lastvisitor::view-your-profile-again') : get_user_name($notification['from_userid']).' '.lang('lastvisitor::view-your-profile');
        $result['options']['link'] = url_to_pager('lastvisitor', array('id' => $user['username'])).'?jsi='.$user['id'];
        $result['type'] = $type;
        $result['status'] = true;
